...
<head>
    ...
    <style>
        ...
        /* Layout debug outlines (toggle via body.layout-debug) */
        body.layout-debug #topology-container { outline: 2px dashed rgba(255, 0, 255, 0.6); outline-offset: 4px; }
        body.layout-debug #topology { outline: 2px dashed rgba(0, 255, 255, 0.6); outline-offset: -2px; }
        body.layout-debug #status { outline: 2px dashed rgba(255, 165, 0, 0.6); outline-offset: 4px; }
        body.layout-debug #controls { outline: 2px dashed rgba(76, 175, 80, 0.6); outline-offset: 4px; }
        body.layout-debug .status-card { outline: 1px dotted rgba(200, 200, 200, 0.7); outline-offset: 2px; }
        body.layout-debug .worker-modal-content { outline: 2px dashed rgba(244, 67, 54, 0.7); outline-offset: 2px; }
        .status-card {
...
<body class="layout-debug">
    <div style="display:flex; justify-content: space-between; align-items: center; gap: 12px;">
        <h2 style="margin: 0;">Replication Monitor</h2>
        <button id="toggle-layout-debug" class="btn" style="padding: 4px 8px; font-size: 12px;" title="Toggle layout borders/outlines">Layout borders</button>
    </div>
    ...
    <script>
        ...
        document.getElementById('toggle-layout-debug').addEventListener('click', () => {
            document.body.classList.toggle('layout-debug');
            // Re-render the topology so bounds boxes update immediately
            // @ts-ignore
            if (window.__mydbaLastReplicationMessage && typeof renderTopology === 'function') {
                // @ts-ignore
                renderTopology(window.__mydbaLastReplicationMessage);
            }
        });
        ...
        window.addEventListener('message', event => {
            ...
            if (message.command === 'replicationStatus') {
                // Cache latest for debug toggle re-render
                // @ts-ignore
                window.__mydbaLastReplicationMessage = message;
