...
try {
    ...
    // Avoid noisy logs; this is only an issue if the user clicks worker details.
}
...
function renderTopology(data) {
    ...
    container.innerHTML = '';
    const debugLayout =
        (document && document.body && document.body.classList && document.body.classList.contains('layout-debug')) || false;
    ...
    if (debugLayout) {
        // Outline the SVG to see if inner elements exceed it
        svg.style('outline', '2px dashed rgba(255, 0, 255, 0.75)');
        svg.style('outline-offset', '-2px');
    }
    ...
    // Visual bounds to debug card clamping vs. viewport (only when layout-debug is on)
    if (debugLayout) {
        const debugLayer = g.append('g').attr('pointer-events', 'none');
        ...
        debugLayer.append('rect')
            ...
            .attr('height', height)
            .attr('fill', 'none')
            .attr('stroke', 'rgba(0, 255, 255, 0.75)')
            .attr('stroke-width', 1)
            .attr('stroke-dasharray', '4,3');

        // Clamped safe area based on card padding
        debugLayer.append('rect')
            .attr('x', CARD_PADDING_X)
            .attr('y', CARD_PADDING_Y)
            .attr('width', Math.max(0, width - CARD_PADDING_X * 2))
            .attr('height', Math.max(0, height - CARD_PADDING_Y * 2))
            .attr('fill', 'none')
            .attr('stroke', 'rgba(255, 165, 0, 0.85)')
            .attr('stroke-width', 1)
    ...
    // Debug rectangles around each node/card to see what exceeds the viewport
    if (debugLayout) {
        node.append('rect')
            .attr('x', -CARD_WIDTH / 2)
            ...
            .attr('fill', 'none')
            .attr('stroke', 'rgba(255, 0, 255, 0.7)')
            .attr('stroke-width', 1)
...
function showWorkerDetails(node) {
    if (!vscodeApi) {
        // Only log when the user clicks the icon; this indicates host didn't provide the API
        console.warn('[ReplicationTopology] VS Code API not available; cannot request worker details');
        return;
    }

    // Request worker data from extension
    vscodeApi.postMessage({ command: 'getWorkers' });
